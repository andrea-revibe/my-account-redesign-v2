...
- `src/components/` — one component per file. `InProgressCard`, `OrderCard`, `PastOrderCard`, `ClaimCard` are the four baseline order-card variants; `DocsRejectedCard`, `PickupFailedCard`, and `InvalidClaimCard` are takeover variants that supersede `ClaimCard` when the claim is blocked on a single customer action (re-upload evidence / confirm new pickup / pay return shipping after an invalid claim). `WarrantyClaimCard` is a sibling of `ClaimCard` routed on `claim.type === 'warranty'` — same chrome, different post-QC tail (repair-and-ship-back). `ClaimDetailsSheet` is `ClaimCard`'s details sheet (warranty-aware: hides the Refund destination row and renders a Return section for warranty claims). `ClaimActionBanner` is a private child of `ClaimCard` rendered inside its expanded state when `claim.actionRequired` is set; the `See detailed tracking` dropdown (rendered when `claim.transitSubTimeline?.picked_up` is set on `ClaimCard`, or when `claim.shipBack?.awb` is set on `WarrantyClaimCard`) is a private helper inside each card. `UndoSnackbar` is a demo-only toast surfaced by `App.jsx` after the returns flow submits an in-session claim — auto-dismisses after 8s; Undo deletes the seeded claim from `submittedClaims`. See `docs/output/returns/claim_tracking.md` and `docs/output/warranties_compensations.md`. `WalletInfoTooltip` is shared anywhere "Revibe Wallet" is named — reuse it; it exports `REVIBE_WALLET_ICON`.
- `src/components/ClaimFlow/` — returns flow with three wired branches. `change_of_mind` and `issue` are 7 steps; `warranty` is 6 (Step 5 refund-method is skipped — see `visibleStepCount` / `visibleStepIndex` in `flowReducer.js`). `ClaimFlow.jsx` mounts conditionally (no `open` prop — unmounting is what resets state) and on submit calls `onSubmitClaim(orderId, claim)` to seed the order at the App level. `flowReducer.js` owns the state shape + per-step `canAdvance`. Step 2 reuses `Step2IssueDetails` for both `issue` and `warranty`; the refund-method screen is skipped for warranty in NEXT/BACK/GO_TO_STEP. Details in `docs/output/returns/change_of_mind.md`, `docs/output/returns/issue.md`, and `docs/output/warranties_compensations.md` §2.
...
- `src/lib/claims.js` — single source of truth for `ClaimCard` **and** `WarrantyClaimCard`: states, tone, copy helpers, `hasActiveClaim` / `isClaimRefunded` / `isWarrantyDelivered`. Hosts the refund pipeline (`CLAIM_STATUSES` + `claimToneFor` / `claimProgressIndex` / `claimPhaseTag`) and the warranty pipeline (`WARRANTY_CLAIM_STATUSES` + `warrantyClaimToneFor` / `warrantyClaimProgressIndex` / `warrantyClaimPhaseTag`). Also hosts `CLAIM_TRANSIT_SUB_STATUSES` + `transitSubProgressIndex` (driving `ClaimCard`'s `See detailed tracking` dropdown when `claim.transitSubTimeline?.picked_up` is set — `WarrantyClaimCard`'s dropdown reuses `SHIPPING_SUB_STATUSES` from `lib/statuses.js` instead), `SUB_STATUS_LABELS` (customer-facing copy registry for each sub-status — kept for ops/internal references and any future inline surface), `CLAIM_SLAS` (SLA placeholders covering both refund and warranty-tail steps, used by the Step 4 detailed-timeline dropdown), `expectedCompletionFor(claimType)` (sums SLA `expectedHours` across the pipeline and returns `{ short, long, hours, date }` for the Step 4 "Expected by" headline + warranty Review / Confirmation surfaces), and `actionGateCopy` (banner copy for the three action gates).
- `src/lib/events.js` — `getHistoryEvents(order, mode)` drives the `HistoryThread` on layered cards.
- `src/data/journey.js` — `INITIAL_ORDER` + the `JOURNEYS` array (today: `happy_path`, `cancel_at_qc`, `claim_change_of_mind`, `claim_warranty`). Each node: `{ id, label, trigger, event, apply(order), next? }`. Branching is opt-in via `next: [...nodeIds]`; omitting it defaults to next-in-array. Terminal nodes set `next: []` if not last in the array.
- `src/lib/journey.js` — `useJourney(journeyId)` hook. Path-based replay (`path: string[]`) so branched journeys resolve correctly. Exposes `validNext` / `advance` / `back` / `reset`.
- `src/components/JourneyDevPanel.jsx` — bottom-right floating dev tool, journey-mode only. Picker chip row, step counter + back chevron, one `Next` button per `validNext()` option (customer-triggered styled outlined with a `via UI` chip; system-triggered solid brand). Exits journey mode via the header chip, not the panel.
...
- **Card routing.** `App.jsx` picks in order: `claim.docsRejection` → `DocsRejectedCard`; `claim.pickupFailure` → `PickupFailedCard`; `claim.invalidClaim` → `InvalidClaimCard`; `claim.type === 'warranty'` + active → `WarrantyClaimCard` (In Progress); `hasActiveClaim` → `ClaimCard` (In Progress); `isWarrantyDelivered` → `WarrantyClaimCard` (Past); `isClaimRefunded` → `ClaimCard` (Past); else by `statusId`/`state`: `InProgressCard` (created/QC), `OrderCard` (shipped + in-flight cancelled), `PastOrderCard` (delivered without a claim, or cancelled-and-refunded). The first three are takeover cards — they replace the baseline card's surface while the claim is blocked on a single customer action and auto-cancels (or auto-closes) if ignored. `WarrantyClaimCard` is not a takeover; it's a sibling of `ClaimCard` for warranty claims with a repair-and-ship-back tail instead of a refund tail. Full tree in `docs/output/orders.md` §2.
...
- **Returns flow.** Overlay launched from `Raise a claim` on the delivered `PastOrderCard`. State in one `useReducer`; no session persistence (closing unmounts). Three wired branches: `change_of_mind` and `issue` are 7 steps and diverge at Step 2 (Reason vs Issue picker); `warranty` reuses the Issue picker on Step 2 and **skips Step 5** (refund-method) — total 6 visible steps, progress bar driven by `visibleStepCount(claimType)`. Eligibility + refund math in `src/lib/returns.js`; pipeline + expected-completion in `src/lib/claims.js`. Submit calls `onSubmitClaim(orderId, claim)` so `App.jsx` can stitch the seeded claim onto the order (in-session only — cleared on refresh; undoable via the `UndoSnackbar`). Chrome is deliberately checkout-style, **not** the order-card chrome family. See `docs/output/returns/change_of_mind.md`, `docs/output/returns/issue.md`, and `docs/output/warranties_compensations.md` §2.
...
- **Many things are visual placeholders.** Search, Wallet pill balance, profile menu, language toggle, "Download receipt" — all decorative. "Raise a claim" is wired for the three real branches (`change_of_mind`, `issue`, `warranty`); the **`Request compensation`** entry on Step 1 is still stubbed. Step 6's submit seeds an in-session claim on the order (cleared on refresh, undoable via `UndoSnackbar`) — there's no backend. The `WarrantyClaimCard` also has two hand-seeded mocks (89610 / 89580) that exercise the `under_repair` and `ship_back` heroes — submit-time seeding always lands on `initiated` and can't progress beyond that. Per-feature mocked-vs-prod lists in each `docs/output/*.md`.
