...
- `src/lib/claims.js` — single source of truth for `ClaimCard`: states, tone, copy helpers, `hasActiveClaim` / `isClaimRefunded`. Also hosts `SUB_STATUS_LABELS` (customer-facing copy for each sub-status, used by the inline `expert_revision` notes), `CLAIM_SLAS` (SLA placeholders, used by the Step 4 "What happens next" timeline), and `actionGateCopy` (banner copy for the three action gates).
...
- **Detailed claim tracking.** Two layers inside `ClaimCard`'s expanded state. `ClaimActionBanner` is the always-visible inline gate when `claim.actionRequired` is set (collection failed, missing documents, return-shipping payment). Inline sub-status notes above the dot strip surface QC sub-flow context that the dots can't show — today only `subStatusId === 'expert_revision'` triggers them, rendering a past `Reviewing seller's response` callout above an active `Expert inspection` callout. Copy sourced from `SUB_STATUS_LABELS` in `lib/claims.js`. Canonical reference: `docs/output/returns/claim_tracking.md` §4. Design history: `docs/output/claim_detailed_tracking.md`.
...
Full doc map and conventions: `docs/README.md`. For `CHANGELOG.md`, add to the top `[Unreleased]` block — keep bullets to 1–2 sentences; the diff shows what changed, the bullet just names it. Update repo `README.md` only if "Where things live" or "Scope reminder" needs adjustment.
