...
- **Claim pipeline.** 5 states `initiated → pickup → qc → refund_issued → refund_credited`, tone warn→brand→success; `hasActiveClaim` until `refund_credited`. Compensation = parallel 4-state chain (minus Pickup). `claim_tracking.md` §1.
- **Claim resolution date.** A computed *window* off `claim.milestones` that **ends at the verdict**; `claimErdFor` owns all suppression. **Trap:** it rebases *later* at pickup. `claim_tracking.md` §11.
...
- **Warranty coverage.** Tier sets duration + what qualifies: standard = defects 12mo; Revibe Care (`order.warranty > 0`) = + accidental damage, 24mo, capped, one use (acked at submit → `claim.accidentalAck`). **Trap:** ages run from the *order* date, not delivery. `warranties_compensations.md` §4.
- **Over-cap repair quote.** `RepairQuoteCard` (6th takeover, `claim.repairQuote`): pay the excess or take the device back free. **Trap:** a *pause*, not a state — claim stays at `qc`; `repairDeclined` drops `under_repair`. `warranties_compensations.md` §5.
- **Returns flow.** Overlay from the delivered `PastOrderCard`; one `useReducer` walking `sequenceFor(state)`. **Trap:** situation/reason tripwires (`tripwireFor`) open `SwitchFlowSheet` with no "continue anyway", so a faulty device can't ride the change-of-mind track. `change_of_mind.md` / `issue.md`.
- **Cancel claim.** `canCancelClaim(claim)` gates a `Cancel claim` footer through `initiated`/`pickup`/`qc`. **Fork:** `cancelNeedsShipBack(claim)` → clean revert (pre-pickup + compensation; undoable) vs ship-back (device collected → `invalidClaim` gate, `reason: 'cancelled'`). `claim_tracking.md` §2.8.
- **Invalid-claim ship-back.** `InvalidClaimCard` = "device at Revibe, pay return shipping," keyed by `claim.invalidClaim`; `.reason` discriminates `'invalid'` (QC verdict) vs `'cancelled'` (post-collection cancel). `claim_tracking.md` §3.3.
- **Reset-failed iCloud unlink.** `ResetFailedCard` takes over (danger) on Activation Lock until iCloud unlink + passcode; `action_needed → submitted`, iOS-only copy. `claim_tracking.md` §3.4.
- **Journey mode.** Opt-in demo (`?journey=<id>`) replaying an order; branching via `node.next` (per-edge guards `countries` / `when(order)`). Copy is event-keyed data in `data/notifications/*` with a coverage `status` → badge. **Trap:** replays run frozen calendars, so date math must default to `orderAsOf(order)`, never `new Date()`. `journey_backend_spec.md`.
- **Sandbox journeys.** Parameter-driven variants (`kind: '*_sandbox'`, `nodes: []`) each pair a hook + panel; `App.jsx` picks by kind. `journey_backend_spec.md`.
- **Country split.** Orthogonal dimension, not a journey/component fork: `order.country` (`AE`/`ZA`/`SA`/`Others`, default AE) → `countryConfig` flags (`lib/countries.js`) gate shared cards; sequence forks use per-edge `next` country tags filtered in `validNext`. `country_split.md`.
- **Order↔claim linkage.** Every claim card wraps in `OrderClaimLink` (one-open order↔claim accordion); refs render typed via `formatClaimRef`. `OriginalOrderSheet` is retired. `claim_tracking.md` §10.
- **Revibe Wallet.** `GreetRow` pill / wallet chip → `WalletSheet`; `lib/wallet.js` ledger = seed (`data/wallet.js`) + live refund, live pinned above seed. **Trap:** Move-to-card re-applies the waived deduction (zero for Revibe/compensation), latest refund only — funds-gated, no cascade, undoable. `wallet.md`.
...
- Reuse `ClaimErdStrip` (`src/components/ClaimErdStrip.jsx`, props `{ erd, toneText, className? }`) for the estimated-resolution window in any claim-card hero — pass `claimErdFor(order)` (`lib/claimErd.js`) straight through **unconditionally**: it returns `null` for every state that shouldn't show a date (action gates, repair quotes, post-verdict, pending), so suppression stays in one place instead of spreading into card guards. Don't add a second date strip pre-verdict; post-verdict the card's own tail (`RepairWindowStrip`, refund block) owns the date. When a stage has no anchor, `claimErdFor` stands today in for the outstanding milestone (`assume: 'when_pending'`) so there's always a date — but it **never advances the reported stage** and **never assumes the verdict**, and it stays silent for a claim carrying no `milestones` block at all. Spec: `docs/output/returns/claim_tracking.md` §11 + §11.3a.
- Reuse `StatusExplainer` (`src/components/StatusExplainer.jsx`, props `{ pill, explanation, label?, className? }`) for the inline "ⓘ Learn more" stage-definition affordance beside a status chip — pass the chip via `pill` (it owns the `[chip · Learn more]` row + the full-width reveal below) and the copy via `explanation`. Resolve copy from the source-of-truth libs (`statusExplanation` in `lib/statuses.js`; `claimExplanation` / `warrantyClaimExplanation` in `lib/claims.js`) — don't hardcode strings in the card. Renders the bare `pill` when `explanation` is null, so route the chip through it unconditionally. Only add it where the header doesn't already define the stage (skip self-explaining heroes / bannered surfaces).
...
- Reuse `AddressForm` (`src/components/AddressForm.jsx`, props `{ address, country, onChange, errorField? }`) for **every** editable postal address — it renders the country's structured field schema from `lib/address.js` (`addressSchema` / `emptyAddress` / `formatAddress` / `addressError`); `onChange` returns the whole next address object (callers shallow-merge on `address`). Render a stored address via `formatAddress(addr, country)` (string-tolerant — pass legacy strings through) and soft-validate via `addressError`; don't re-roll per-field address inputs. Country schemas + which surfaces use it: `docs/output/country_split.md` §4b.
...
- **Never leave a dev-panel date input empty.** A native `<input type="date">` with no value opens its calendar on the *real* current month, and there is no attribute that changes that — so on a sandbox dated 2026-05 the picker lands months from every other field. Both panels therefore always hand the input a date (its own, or the derived `anchorDate` = latest committed date) and use a **checkbox** to mean "not reached yet". Don't reintroduce a nullable date input. Also unverifiable by test: the picker is browser chrome, not DOM.
...
| Claim resolution date — `lib/claimErd.js` levers/stages, `claim.milestones`, `ClaimErdStrip`, the `claim_erd` sandbox | `docs/output/returns/claim_tracking.md` §11 + `CHANGELOG.md` |
