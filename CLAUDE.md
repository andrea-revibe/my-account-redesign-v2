...
- **Order↔claim linkage.** Every claim card wraps in shared `OrderClaimLink` (parent order strip + connector thread → tap opens `OriginalOrderSheet`, whose reciprocal "Linked claim" row rounds back to the card via `onReveal` + a `ringPulse` highlight); the strip owns the order #, and refs render typed via `formatClaimRef` (`RET`/`WAR`/`CMP`/`CXL`). `claim_tracking.md` §10.
...
- Reuse `OrderClaimLink` (`src/components/OrderClaimLink.jsx`, props `{ order, onReveal, children }`) to give any claim-card-family member its order linkage — wrap each `<article>` return; it renders the parent order strip + connector thread + `OriginalOrderSheet` + the round-trip ring, so don't re-roll them. Render every claim ref through `formatClaimRef(claim)` (`lib/claims.js`) — never hardcode a `RET-` prefix (the takeover-card eyebrows used to). Design: handoff bundle `Order-Claim ID Pairing`; spec `claim_tracking.md` §10.
