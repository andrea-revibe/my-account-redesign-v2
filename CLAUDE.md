...
- **Sandbox journeys.** Parameter-driven (not event-driven) journey variant: `kind: 'sandbox'` + `nodes: []`, paired with its own hook + panel (`useEddSandbox` / `EddSandboxPanel`), feeding the unchanged card-routing tree. Injects banner copy via `order.statusBanner: { tone, lead, body }` (a full override of `statusDescription` when set, unlike `statusMessage` which overrides body only). The sandbox sets it **only for SLA-divergence messages** (late at any stage + `qc_back_on_track`); the on-time happy path + `delivered` leave it null and reuse the card copy (`STATUS_DESCRIPTIONS`) so the two can't drift. First example `dynamic_edd`. See `journey_backend_spec.md`.
...
- **Status banner tone resolution.** `statusDescription(order)` resolves tone in order: cancelled → red "Refund in progress"; `delayed` → orange "Taking longer than expected" (full warn-amber banner on `OrderCard`; on `InProgressCard` only the `Clock` tag accent goes amber — `text-amber-600` — while the hero stays brand-purple); else `STATUS_DESCRIPTIONS[statusId]`. `statusMessage` overrides body only. Full table: `orders.md` §4.5.
...
- **Returns flow.** Overlay launched from `Raise a claim` on a delivered `PastOrderCard`; one `useReducer`, no session persistence (closing unmounts). Four wired branches; `state.step` is a **string key** and each claim type walks its own ordered `STEP_SEQUENCES[claimType]` list in `flowReducer.js` (`NEXT`/`BACK`/`GO_TO_STEP` walk it — no skip arithmetic). `change_of_mind` 8 steps, `issue` 9, `warranty` 8 (no refund step), `compensation` 5 (no reason/device/packing/pickup, amount deferred, no ack gate). **Shared reason step → routing safety net:** the required `'reason'` step (`Step2Reason`, "Why are you returning it?") is shared by change-of-mind / issue / warranty and is the authoritative router — `routeForReason` resolves each reason to a track; a mismatch opens the **authoritative `SwitchFlowSheet`** (no "continue anyway") to redirect (fault/wrong → issue, or warranty if Revibe Care; `missing_parts` → compensation; no-fault → change of mind). The point is that a faulty device must never proceed down the no-fault change-of-mind track. **Flow-wide soft validation: the Continue/Submit button is never disabled** — clicking with a required input missing dispatches `ATTEMPT` and `stepError(state)` lights up the first unmet field inline (`InlineError`) instead of advancing; the flag is reducer-owned, cleared by every step-changing action (incl. `ROUTE_FROM_REASON`). Submit calls `onSubmitClaim(orderId, claim)` to seed an in-session claim (cleared on refresh; undoable via `UndoSnackbar`). Checkout-style chrome, not the order-card family. See `docs/output/returns/change_of_mind.md`, `issue.md`, and `warranties_compensations.md` §2–3.
...
- **Invalid-claim ship-back.** `InvalidClaimCard` is the generic "device is at Revibe, pay return shipping to get it back" surface, keyed by `claim.invalidClaim`. `claim.invalidClaim.reason` discriminates the trigger: `'invalid'` (QC verdict — default) vs `'cancelled'` (customer cancel post-collection, see Cancel claim). In-component states `action_needed → paid | declined` (`declined` is invalid-only; cancel uses `Keep claim` → resume). A compensation invalid short-circuits to `CompensationClosedCard` (no ship-back gate). `docs/output/returns/claim_tracking.md` §3.3.
- **Cancel claim.** `canCancelClaim(claim)` (`lib/claims.js`) gates a persistent `Cancel claim` footer on `ClaimCard` / `WarrantyClaimCard` (visible collapsed or expanded) — cancellable through the whole pre-completion window (`initiated`/`pickup`/`qc`); takeover cards expose it regardless. `cancelNeedsShipBack(claim)` then picks the path: **clean revert** (pre-pickup device claims + all compensation) strips the claim → delivered `PastOrderCard` (`App.cancelledClaims` / `claim_cancelled` node, undoable); **ship-back** (device already collected) overlays the `invalidClaim` gate with `reason: 'cancelled'` (`cancelReturnGate`, `App.shipBackCancels` / `claim_cancelled_shipback` node) → `InvalidClaimCard` pay-return-shipping, reusing the invalid return chain. `CancelClaimSheet` copy is path-aware. `docs/output/returns/claim_tracking.md` §2.8.
