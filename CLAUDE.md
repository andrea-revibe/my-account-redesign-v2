...
- `src/App.jsx` — page composition; owns filter state (`activeStatus`, `activeRange`) and computes `activeId` for the auto-expand rule. Routes orders to one of four cards (see Mental models → "Card routing"). Also owns `claimFlowOrderId` and conditionally mounts `<ClaimFlow />` above the account view.
- `src/components/` — one component per file. `OrderCard` contains its own sub-components (`SummaryHeader`, `ProductRow`, `OrderIdRow`). `InProgressCard`, `PastOrderCard` (delivered + cancelled-past variants), and `ClaimCard` all carry the "refund-hero family" chrome — share a left accent strip + `Order · #id` eyebrow + state pill row + tinted hero block + compact product row, differing in tone/headline/footer per state. `ClaimDetailsSheet` is the bottom sheet behind `ClaimCard`'s `View claim details` button (mirrors `RefundDetailsSheet`'s pattern). `WalletInfoTooltip` is shared: anywhere "Revibe Wallet" is named (credits pill in `GreetRow`, refund option + confirm-step destination in `CancelOrderSheet`, refund option in `Step7RefundMethod`), reuse it instead of building another tooltip — it exports `REVIBE_WALLET_ICON` too.
- `src/components/ClaimFlow/` — the seven-step change-of-mind returns flow. Mounted conditionally by `App.jsx` (no `open` prop — conditional render is what resets state, since the brief forbids session persistence). `ClaimFlow.jsx` is the overlay shell; `flowReducer.js` owns the state shape + per-step `canAdvance` validation; one `StepN*.jsx` file per step. Visual chrome is deliberately checkout-style (white surface, segmented progress bar, sticky brand-purple action bar, line-bordered selection cards) — distinct from the order-card chrome family so the user feels they've entered a different *mode*. See Mental models → "Returns flow".
- `src/data/orders.js` — eight mock orders covering all states. Optional fields: `delayed: true` triggers the warn-tone status banner (`OrderCard` only — `InProgressCard` ignores it for the hero, see Mental models); `statusMessage: '…'` overrides the banner body; `estimatedDeliveryLong` / `deliveredOnLong` give the hero its long-form date headline. Returns-flow-only fields (`deliveredOn`, `unitPrice`, `paymentMethod`, `deviceOs`) are populated on the delivered iPhone 13 (`89657`) only — the one eligible order — and absent on the rest (which fall into "Not eligible" naturally). The optional `claim` object on an order shifts it from a delivered `PastOrderCard` to a `ClaimCard`; today only `89219` (Pixel 6) carries one, seeded in the `under_qc` state. See `docs/my-account-flow.md` § 4.8 for the shape.
...
- `src/lib/claims.js` — single source of truth for `ClaimCard`: `CLAIM_STATUSES` (the 7-state ordered list), `claimToneFor`, `claimProgressIndex`, `claimPhaseTag`, `claimStatusHeadline`, `claimStatusSubline`, `hasActiveClaim` / `isClaimRefunded` (used by `App.jsx` for routing + filter counts), plus the summary-label helpers (`REASON_LABELS`, `RETURN_METHOD_LABELS`, `reasonText`, `devicePrepText`, `refundMethodLabel`). Edit claim copy / states / tone mapping here, not in the components.
...
**Card routing.** Four card components, picked in `App.jsx` in this order (first match wins for delivered orders): `hasActiveClaim(order)` → `ClaimCard` in In Progress; `isClaimRefunded(order)` → `ClaimCard` in Past; else the existing routing.
...
- `PastOrderCard` — `statusId === 'delivered'` *(without a `claim` field)* (delivered branch, no expand) and `state === 'cancelled' && cancellationStatusId === 'refunded'` (cancelled-past refund-hero variant). Same component, two render branches based on `order.state`.
- `ClaimCard` — any order carrying `order.claim`. Replaces the delivered card. Tone follows `claimToneFor` (amber for the first five states, brand-purple for `ready_for_refund`, success-green for `refunded`); lives in In Progress while active and drops to Past once refunded. `View claim details` opens `ClaimDetailsSheet` — the summary fields live in the sheet, not inline in the expanded card.
...
**Returns flow.** Seven-step overlay (`src/components/ClaimFlow/`) launched from the `Raise a claim` button on the delivered `PastOrderCard`. State lives in a single `useReducer` in `ClaimFlow.jsx`; per-step validation is `canAdvance(state)` in `flowReducer.js`. Three things to know: (1) **No session persistence** — closing the overlay unmounts the component (`App.jsx` renders it conditionally, not via an `open` prop), so reopening always starts fresh. (2) **Entry pre-seeds state but the flow always starts at Step 1** — `initialState(orderId)` pre-seeds `orderId` and `claimType: 'change_of_mind'`, so confirming Step 1 advances straight to the reason step (Step 2). There's no order/product picker inside the flow — the delivered card is product-specific, so the item being returned is implied by the entry point. (3) **Eligibility, refund math, and fee rate all live in `src/lib/returns.js`** — change them there, not in the step components. The flow's chrome is deliberately checkout-style and does NOT use the order-card chrome family (no accent strips, no eyebrows, no tinted hero blocks except the Step 3 warn callout). See `docs/my-account-flow.md` § 2.7.
