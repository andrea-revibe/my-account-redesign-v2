...
- **Journey mode.** Opt-in demo (`?journey=<id>`) replaying an order via `JourneyDevPanel`; multi-journey + branching in `JOURNEYS` / `node.next`. Replay stacks a `JourneyNotificationPanel`; copy is event-keyed data in `data/notifications/*`, each entry carrying a coverage `status` → badge + roll-up. `journey_backend_spec.md`.
...
- **Order↔claim linkage.** Every claim card wraps in shared `OrderClaimLink` — a one-open accordion pairing the order half and the claim/cancellation half on a connector rail; the collapsed half is a `View ▸` compact row, and refs render typed via `formatClaimRef` (`RET`/`WAR`/`CMP`/`CXL`). `OriginalOrderSheet` retired — the expanded `PastOrderCard` is the only order surface. `claim_tracking.md` §10.
