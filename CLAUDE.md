...
- **Card routing.** `App.jsx` picks one card by precedence: claim takeovers (`docsRejection → awbFailure → pickupFailure → resetFailed → invalidClaim`) → `WarrantyClaimCard` (sibling, not takeover) → `hasActiveClaim` (`ClaimCard`, incl. active compensation) → past (`isWarrantyDelivered` / `isClaimRefunded` / `isClaimClosed`) → baselines (`InProgressCard` / `OrderCard` / `PastOrderCard`). Full tree: `docs/output/diagrams.md#card-routing` + `orders.md` §2.
...
- **Warranty coverage.** Tier sets duration + what qualifies: standard = defects 12mo; Revibe Care (`order.warranty > 0`) = + accidental damage, 24mo, capped, one use. **Trap:** ages run from the *order* date, not delivery. `warranties_compensations.md` §4.
...
- **Detailed claim tracking.** Inside `ClaimCard` expanded: `ClaimActionBanner` gate, Initiated scheduled-pickup strip, opt-in `See detailed tracking` dropdown (gated on `claim.transitSubTimeline.picked_up`). `claim_tracking.md` §2.2.
