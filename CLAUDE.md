...
- **History thread.** On layered cards, past events render as compact chips under the hero; tap expands one inline (`lib/events.js`). `orders.md` §6.
...
- **Sandbox journeys.** Parameter-driven journey variant (`kind: 'sandbox'`) with `useEddSandbox` / `EddSandboxPanel`; injects `order.statusBanner` only for SLA-divergence messages. `journey_backend_spec.md`.
