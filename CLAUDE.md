...
- **Card routing.** `App.jsx` picks one of eight cards by precedence: claim takeovers (`docsRejection → pickupFailure → resetFailed → invalidClaim`) → `WarrantyClaimCard` (sibling, not takeover) → `hasActiveClaim` (`ClaimCard`, incl. active compensation) → past (`isWarrantyDelivered` / `isClaimRefunded` / `isClaimClosed`) → baselines (`InProgressCard` / `OrderCard` / `PastOrderCard`). Full tree: `docs/output/diagrams.md#card-routing` + `orders.md` §2.
