...
- `src/App.jsx` — page composition; owns filter state, `claimFlowOrderId`, and the in-session `submittedClaims` map (orderId → claim) that's projected over `ORDERS` so a freshly-submitted claim flips the order to its claim-card variant immediately. Routes orders to one of eight cards (see Card routing).
