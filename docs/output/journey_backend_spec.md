...
Source of truth: `src/data/journey.js → JOURNEYS`. Today the array holds six: `happy_path`, `dynamic_edd`, `cancel_at_qc`, `claim_change_of_mind`, `claim_issue`, `claim_warranty`. `?journey=1` aliases the first journey; unknown ids fall back to the first.
