...
Customers can cancel orders themselves from the `created` and `quality_check` states. A `shipped` order normally needs support — with two exceptions, both raised from the delivery hero: a shipment that has **stalled in transit past the cancellation window** (§2.6, outside AE only — it needs a courier recall), and one the customer **refused at the door** (§2.7, every market — the parcel is already coming back). Cancellation has two surfaces:
...
### 2.7 Refused-delivery cancellation (`shipped`, **all markets**)

The second shipped exception. The courier arrives, the customer turns the parcel away, and it heads back to us — at which point the order needs the same self-cancellation affordance as a stalled one, because the money question is identical: *the device isn't coming, how do I get paid back?*

**The gate** — the same `canCancelShipped(order)`, restructured into two ways in rather than three joint conditions:

```js
if (order.statusId !== 'shipped' || order.state === 'cancelled') return false
if (order.deliveryRefused === true) return true          // every market, incl. AE
if (!countryConfig(order).shippedCancellation) return false
return order.promiseBreached === true                    // stalled path — country-gated
```

**The refusal path is not country-gated**, unlike §2.6. `shippedCancellation` encodes whether a market can have a *moving* parcel pulled back by the courier — a real operational capability AE doesn't offer. A refused parcel is already travelling back on its own, so there's nothing to arrange and no capability question to ask: AE can self-cancel a refused delivery. (This also keeps the refusal banner honest — it tells the customer to cancel, and now every market can.) The surface is unchanged: `HeroCard`'s live `Cancel order` hosting `CancelOrderSheet`.

**Why a second flag instead of reusing `promiseBreached`.** `promiseBreached` means *we broke the delivery promise*, and it carries an apology's worth of behaviour: the AED 100 `LATE_PROMISE_WALLET_BONUS`, and the "We missed our delivery estimate" dissuade screen. Neither is true of a refusal — we turned up, the customer said no. So `deliveryRefused` is separate, and the sheet splits the money from the apology: `feeWaived = breached || refused` drives the fee, `breached` alone drives the bonus + apology copy.

**The terms** — full refund, **5% processing fee waived** (the customer never took the device), **no Wallet bonus**, either destination. The dissuade step is **skipped entirely** on the refusal path (`dissuadeEligible = DISSUADE_STATUSES.has(statusId) && !refused`): the retention pitch has nothing to offer once the parcel is already travelling back, and the apology variant would be claiming a delay that didn't happen. Select → Confirm, both refund methods.

**The request is reviewed, not auto-accepted** — `cancellationStatusId: 'requested'`, a 48-hour confirmation window, same as §2.6. The reason differs: nothing has to be *recalled*, so the confirmation is ours to give rather than the courier's to attempt. Only one outcome is modelled: **accepted** → `refund_pending` → `refunded`. There is no declined branch (a refused parcel is already in the courier's return leg — there is no recall that can fail) and no reversal branch (see §9 for what that costs).

**Copy deltas vs the stuck-in-transit sheet**, all keyed on `deliveryRefused` so §2.6 is untouched:

| Surface | Stuck in transit | Refused delivery |
|---|---|---|
| Hero headline | last courier scan (e.g. *Cleared customs*) | *Delivery refused* — via the banner's optional `headline` (see [orders.md](./orders.md) §4.5), because "Out for delivery" over "the parcel is coming back" reads as a bug |
| Hero alert treatment | `warn` — amber triangle, eyebrow stays *Active order* ("we're chasing the courier") | `danger` — solid red *Action needed* pill, red-tinted body block ("cancel, or ask us for another delivery") |
| Confirm info strip tail | *"We'll ask the courier to send the parcel back… if it can't be stopped, your order stays on its way."* | *"The parcel is already on its way back to us — we'll confirm your cancellation within 48 hours."* |
| Step 1 Wallet detail line | *Full refund + bonus · once the recall is confirmed* | *Full refund · once the cancellation is confirmed* |
| Requested-hero caveat row (§3) | *"Subject to the courier stopping your parcel…"* | *"The refused parcel is on its way back to us — we'll confirm within 48 hours."* |
| ⓘ Learn more explainer | `cancellation_{phase}_shipped` | `cancellation_{phase}_refused` — `statusExplanation` tries the `_refused` key first when `deliveryRefused` is set, then the stage key, then the stage-agnostic default |

**Journey wiring** — `refused_delivery` ("Refused delivery"), `src/data/journeys/refusedDelivery.js`. It is the happy-path graph with one extra edge: both chain ends (`shipped_out_for_delivery` for AE/ZA, `shipped_simple` for SA/Others) offer `delivered` **or** `delivery_refused`, so accepting and refusing sit side by side at the door. `delivery_refused` stamps `deliveryRefused`, drops `estimatedDelivery`, and sets the refusal `statusBanner` — **`tone: 'danger'`**, not the stall's `warn`, because the customer has to decide something, which is what earns the hero's red `Action needed` pill ([orders.md](./orders.md) §4.5) — carrying the optional `headline`; like `shipped_stuck` it never touches `subStatusId`, so one node serves both country chains, and it is **country-neutral** — a courier refusal scan happens everywhere, and so does the resulting cancel affordance, so unlike §2.6 there is no AE contrast to replay.

```
… → out for delivery / shipped
     ├─ delivered                                    (unchanged happy path)
     └─ delivery_refused
          ├─ cancel_shipped_wallet → cancel_shipped_accepted_wallet → refunded_shipped_wallet
          └─ cancel_shipped_card   → cancel_shipped_accepted_card   → refunded_shipped_card
```

The `cancel_shipped_*` ids are **reused from §2.6 on purpose** — ids are journey-scoped and `App.jsx`'s `handleCancelOrder` advances whichever `cancel_shipped_${branch}` is in `validNext`, so the real cancel sheet drives this journey with **no `App.jsx` change**. Replay `?journey=refused_delivery` under any country — `ZA`/`AE` for the granular shipping chain, `SA`/`Others` for the collapsed one; the cancel path is live in all four. Its `shipment.delivery_refused` event is unauthored ⇒ silent.

## 3. Refund-hero card (`PastOrderCard` — cancelled branch)

The refund-hero card leads with the **refund** as the visual hero rather than the fulfilment journey. The whole card is wrapped in `OrderClaimLink` (§10 of [claim_tracking.md](./returns/claim_tracking.md)) so a cancelled order, like a claim, gets the paired order half (a neutral `PreCancellationOrderCard`, since the order never arrived) + connector rail; tapping either compact row flips the accordion between the order and the cancellation half. A `w-1` left accent strip carries the phase tone (warn amber for `requested`, brand purple for `refund_pending`, success green for `refunded`). The top eyebrow now reads `#{cancellationRef}` (the cancellation ref — `Cancellation` when absent), since the wrapper's strip already owns the `Order #{id}`; the phase pill sits on its own row below, wrapped in a `StatusExplainer` that shows an inline "ⓘ Learn more" accordion with a plain-language stage definition (copy from `STATUS_EXPLANATIONS` in `lib/statuses.js` under the `cancellation_<cancellationStatusId>` namespace — one entry per phase); then a tinted hero block whose internal eyebrow reads simply `Cancellation` (the ref moved up to the card eyebrow) — parallel to `ClaimCard`'s `Claim · {type}` eyebrow. Below that, a smaller `Refund of` / `Refunded` label sits above the refund amount (`text-[28px]` tabular-nums) and a destination chip — wallet destinations get a brand→accent gradient chip (echoes the `GreetRow` credits pill) that, when an `onOpenWallet` handler is threaded, is a tappable button (chevron, `stopPropagation`) opening the `WalletSheet` ledger (see [wallet.md](./wallet.md)); card destinations get a neutral chip. BNPL destinations (`refund.destination.kind === 'bnpl'`) render the same neutral chip but with just the provider brand as the label (no `•• last4`), and an inline `BnplDisclaimerTooltip` Info-icon (`stopPropagation` because the header is the tap target) opens a popover: "{provider} may charge additional fees on refunded purchases. Check your {provider} account for details." Refunded orders surface a `fundsAvailable` sub-copy line ("Available now in your wallet"); the two earlier phases make no ETA promise. A `requested` cancellation on a stalled **`shipped`** order (§2.6) closes the hero with a dashed-divided warn-tone `Truck` caveat row — *"Subject to the courier stopping your parcel — we'll confirm within 48 hours"* — because on that path the refund is conditional on the recall succeeding; it's gated on `requested && statusId === 'shipped'`, so created/QC cancellations are unchanged. On a **refused** delivery (`deliveryRefused`, §2.7) the same row drops the condition: *"The refused parcel is on its way back to us — we'll confirm within 48 hours."* When the refund carries a late-promise Wallet bonus (`refund.bonus > 0`, see §2.5), a success-tone *"Includes AED 100 Wallet bonus"* line (`Sparkles`) renders under the hero amount on every phase. `RefundDetailsSheet` shows the bonus as a `Subtotal` → `Revibe Wallet bonus +AED 100` (success tone) block parallel to the card-refund fee block, with `Total refund` = `subtotal + bonus`.
...
| Field | Type | Notes |
|---|---|---|
| `deliveryRefused` *(optional)* | boolean | When `true` on a `shipped` order it opens `canCancelShipped` on its own — **bypassing the `shippedCancellation` country flag**, so AE included (§2.7) — and waives the card processing fee. Not interchangeable with `promiseBreached`: no Wallet bonus, no apology dissuade screen, and the refusal-specific copy in the sheet / requested hero / explainer keys off it. In the prototype it's stamped by the `delivery_refused` journey node; **production** reads the courier's refusal scan. |
...
└── lib/
    ...
    ├── returns.js                    SHIPPED_CANCEL_WINDOW_DAYS (7) + canCancelShipped(order) — the §2.6 gate (country-gated promiseBreached) + the §2.7 gate (deliveryRefused, all markets)
    ├── statuses.js                   CANCELLATION_STATUSES + statusExplanation (stage-specific cancellation copy)
...
- **Refused delivery is journey-mode only, and has no reversal.** §2.7 is reachable only via `?journey=refused_delivery` (any country — the refusal path isn't country-gated) — no static mock carries `deliveryRefused`. Two deliberate prototype gaps: (a) the `requested` hero's *I want to keep my order* CTA renders (it's gated purely on `cancellationStatusId === 'requested'`) but the journey defines no `cancellation_kept` node, so tapping it does nothing — the demo is accepted-only by choice; add the node if the reversal needs showing. (b) There's no re-delivery path: production has to decide whether a refused parcel can be re-attempted at all, and what happens to the order if the customer never cancels (today it sits on the refusal banner indefinitely). Production also needs the real policy on who eats the return freight, and whether the fee waiver survives a customer-caused return.
...
- **`I want to keep my order` is a stub outside journey mode.** In the static showcase confirm just closes the sheet. In **journey mode** `onKeep` → `handleKeepOrder` advances the `cancellation_kept` node (reverts to open, voids the refund, resumes fulfilment, leaves a reversed trace) — defined in both the `cancellation` and `shipped_cancellation` journeys. Because the CTA is gated purely on `cancellationStatusId === 'requested'`, any journey that reaches `requested` **must** define a `cancellation_kept` node or the button renders and does nothing — `refused_delivery` is the deliberate exception (see the refused-delivery bullet). The window is `requested`-only — production still needs a reverse-cancellation endpoint and the rule for the wider window (e.g. whether `refund_pending` is ever reversible, irreversible the moment funds are released to the processor).
