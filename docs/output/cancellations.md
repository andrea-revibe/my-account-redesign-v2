...
Customers can cancel orders themselves from the `created` and `quality_check` states. A `shipped` order normally needs support — with one exception, raised from the delivery hero: a shipment that has **stalled in transit past the cancellation window** (§2.6, outside AE only — it needs a courier recall). A **refused delivery** (§2.7) is the inverse case: nobody asks for anything, because the refusal scan cancels the order and starts the refund by itself. Cancellation has two surfaces:
...
`Cancel order` on a `created` or `quality_check` order (or a stuck-in-transit `shipped` one, §2.6 — the only `shipped` route into this sheet, since a refused delivery cancels itself without one, §2.7) opens a bottom sheet with a `max-height: 92vh`, a `black/45` scrim, and a slide-up entrance. Dismissible by tapping the scrim, the X icon, or pressing `Escape`.
...
**The request is reviewed, not auto-accepted.** The parcel is already with the courier, so the cancellation lands on `cancellationStatusId: 'requested'` — the same review shape as the at-QC cancellation, with a different reason for it: we're waiting on a **courier recall**, not on a supplier packing check. The promise is **48 hours**. Three outcomes, all reachable from `requested`:

| Outcome | Order state | Surface |
|---|---|---|
| **Accepted** — courier confirmed the parcel is coming back | stays `cancelled`, → `refund_pending` → `refunded` | refund-hero card, brand → success (§3.1) |
| **Declined** — parcel was too far along to pull back | back to `open`, `cancellationStatusId` cleared, `delayed` restored, `cancellationRejection` + `cancellationTimeline.rejected` stamped | delivery hero again with a *"Cancellation not possible"* `statusBanner`; `Cancel order` is **live again** (`canCancelShipped` only reads stage + country + `promiseBreached`, so a still-stalled parcel can be re-requested). Once the parcel lands, the `Cancel rejected` chip surfaces in the delivered card's `HistoryThread` (§5) |
| **Reversed** — customer used *I want to keep my order* (§4) | back to `open`, stall banner restored, `cancellationReversal` stamped | delivery hero, then `Cancel reversed` on the delivered card |

There is no return leg to model on any path — the recall happens server-side, the customer never handles the box.

**Copy deltas vs the created/QC sheet**, all gated on `order.statusId === 'shipped'` so the other stages are untouched: (a) the Confirm info strip appends *"We'll ask the courier to send the parcel back and confirm within 48 hours — if it can't be stopped, your order stays on its way."*; (b) the Confirm destination ETA drops the instant promise (*"Paid to your Wallet once the cancellation is confirmed"* / *"Refunded to your card 5–10 business days after the cancellation is confirmed"*); (c) Step 1's Wallet detail line reads *"Full refund + bonus · once the recall is confirmed"* instead of *"available instantly"*. Nothing can promise a clock from the moment of tapping, because the recall verdict gates the money.

**Where the 48h promise lives on the cancelled card.** The refund-hero card renders no status banner (`statusDescription` is only read by `HeroCard` / `OrderCard` / `InProgressCard`), so it surfaces in two places instead: a warn-tone `Truck` caveat row inside the requested hero (*"Subject to the courier stopping your parcel — we'll confirm within 48 hours"*, §3) and the ⓘ **Learn more** explainer, which resolves a stage-specific key — `statusExplanation` now prefers `cancellation_{phase}_{statusId}` over the stage-agnostic `cancellation_{phase}` (`STATUS_EXPLANATIONS` in `lib/statuses.js`), so `cancellation_requested_shipped` talks about the courier recall and `cancellation_refund_pending_shipped` says the parcel is coming back rather than "the order won't ship". Every other cancellation stage falls through to the existing copy.
...
```
shipped_stuck
 ├─ cancel_shipped_wallet   (requested — refund carries the AED 100 bonus)
 │    ├─ cancel_shipped_accepted_wallet → refunded_shipped_wallet
 │    ├─ cancel_shipped_declined ─┐
 │    └─ cancellation_kept ───────┤
 ├─ cancel_shipped_card     (requested — no fee, promise already breached)
 │    ├─ cancel_shipped_accepted_card   → refunded_shipped_card
 │    ├─ cancel_shipped_declined ─┤
 │    └─ cancellation_kept ───────┤
 └─ delivered  ←──────────────────┘   (the courier finally lands it, late)
```

`cancel_shipped_declined` is shared by both refund branches — the decline outcome is identical whichever method was picked, because no refund was ever issued. Both hand-back nodes (`declined` / `kept`) terminate at the same `delivered` node, so the late delivery is one step away and the rejection / reversal trace lands on the delivered card. The stall banner is restored on the kept path from a module-level `STUCK_BANNER` const shared with `shipped_stuck`; the declined path sets its own. `delivered` remains reachable directly from `shipped_stuck`, which keeps the stall from reading as an automatic cancellation.

The reversal node is deliberately named `cancellation_kept` — the same id the at-QC journey uses. Node ids are journey-scoped, and `App.jsx`'s `handleKeepOrder` advances that id by name, so reusing it wires the card's *I want to keep my order* CTA (which is gated purely on `cancellationStatusId === 'requested'`) with no `App.jsx` change. Replay with `?journey=shipped_cancellation&country=ZA` for the live path; `country=AE` is the deliberate no-cancel contrast. `App.jsx`'s `handleCancelOrder` gains `cancel_shipped_${branch}` at the head of its `validNext`-gated candidate list.

**Notification coverage gap.** `order.cancellation.requested` in `src/data/notifications/orders.js` uses `variantBy: 'statusId'` with `created` / `quality_check` variants only, so a shipped request misses the axis and `notificationFor` returns status `live` with null copy. A `shipped` variant is owner-authored copy (see [journey_backend_spec.md](./journey_backend_spec.md)). `order.cancellation.declined`'s existing single copy ("Your order is on the way! … we couldn't cancel the order as our seller already shipped it") reads correctly on this path; `.accepted` / `.reverted` are unauthored ⇒ silent.

### 2.7 Refused delivery — the order cancels itself (`shipped`, every market)

The courier arrives, the customer turns the parcel away, the depot scans it — and **that scan cancels the order and starts the refund**. There is no sheet, no refund-method choice, and no request to confirm: by the time the customer opens My Account the order is already on the refund-hero card (§3).

**Why no self-cancel step.** The customer already gave an unambiguous answer at the door, and the parcel is travelling back whatever they do next. Asking them to *also* raise a cancellation is asking them to say no twice, and it leaves an order stuck in limbo if they never get round to it. So the refusal is treated the way a refusal is: as the cancellation.

**What the refusal node stamps** — `deliveryRefused: true`, `estimatedDelivery: null` (the promise is spent), `state: 'cancelled'`, `cancellationRef`, a market-dependent `cancellationStatusId` + matching `cancellationTimeline` stamp, and the `refund` object. Two deliberate omissions:

- **No `cancellationInitiator: 'revibe'`.** That field routes to `RevibeCancellationCard` (§11), a settled, instant, no-timeline apology surface. This cancellation has a refund journey to walk, so it belongs on the refund-hero `PastOrderCard`.
- **No `statusBanner`.** `statusDescription()` checks the banner *before* the cancelled branch, so a banner here would mask the cancellation copy. (The refusal's old danger-tone hero banner is therefore gone — the customer never sees an intermediate "Delivery refused, please cancel" state.)

**The gate is gone.** `canCancelShipped` no longer reads `deliveryRefused`; a refused order is always already `cancelled`, which the predicate's second line rejects anyway. §2.6's stalled parcel is now the only shipped self-cancel path:

```js
if (order.statusId !== 'shipped' || order.state === 'cancelled') return false
if (!countryConfig(order).shippedCancellation) return false
return order.promiseBreached === true      // stalled path — country-gated
```

`CancelOrderSheet` lost its refusal branch too (`refused`, the dissuade skip, and the two refusal copy variants): the sheet is unreachable on this path, so `feeWaived` is once again just `breached`.

**Market fork — `refusalReview`** (`lib/countries.js`, **SA only**; see [country_split.md](./country_split.md) §4d). The flag asks whether local ops sign the automatic cancellation off before the refund is released:

| Market | Entry `cancellationStatusId` | Timeline |
|---|---|---|
| `SA` (`refusalReview: true`) | `requested` — awaiting local ops sign-off | `Requested → Pending → Refunded` (3 steps) |
| `AE` / `ZA` / `Others` | `refund_pending` — the refund is already moving | `Pending → Refunded` (**2 steps**) |

The step list forks, not just the starting index: `cancellationStepsFor(order)` returns `CANCELLATION_STATUSES.slice(1)` for a refusal outside a `refusalReview` market, so `Requested` is **dropped rather than shown pre-completed** — nobody requested anything, and a ticked "Requested" step would say otherwise. Because the rendered list can now be shorter than the constant, `cancellationProgressIndex(order)` takes the **order** and searches `cancellationStepsFor(order)`; indexing `CANCELLATION_STATUSES` put a two-step timeline one row out.

SA's `requested` step means **ops sign-off** — not a supplier packing check (§2.2) and not a courier recall (§2.6) — and it deliberately quotes **no timeframe**: nothing is being recalled, so there is no courier window to borrow. It also can't fail; there is no declined branch.

**The terms** — full refund to the **original payment method**, nothing deducted. The 5% processing fee is waived (the customer never took the device); there is no Wallet bonus (no delivery promise was broken — we turned up); and there is **no store-credit option**, because the customer made no choice here and we don't get to move their money somewhere they didn't pick. `refund.destination` is read off `order.paymentMethod`.

**No reversal, by construction.** `I want to keep my order` is suppressed on a refusal at every phase — `canKeep = cancellationStatusId === 'requested' && !order.deliveryRefused`. The customer never asked for the cancellation and the parcel is already coming back, so there is no order left to keep. (This retires the deliberate dead click the previous shape had, where the CTA rendered with no `cancellation_kept` node behind it.)

**Refusal-specific copy**, all keyed on `deliveryRefused` so §2.2/§2.6 are untouched:

| Surface | Copy |
|---|---|
| `statusDescription` — `requested` (SA only) | `danger`, lead *Cancellation requested* — *"You turned the parcel away at the door, so we've cancelled the order for you and it's heading back to us. Our local team is signing off on the cancellation, and your refund starts once they do."* |
| `statusDescription` — `refund_pending` | `danger`, lead *Refund pending* — *"…we've cancelled the order for you. Your full refund is on its way back to your original payment method with nothing deducted, and typically lands within 5–7 business days."* |
| ⓘ Learn more explainer | `cancellation_{phase}_refused` — `statusExplanation` prefers the `_refused` key when `deliveryRefused` is set, then the stage key, then the stage-agnostic default |
| Requested-hero caveat row (§3, SA only) | *"The refused parcel is on its way back to us, and our local team is signing the cancellation off."* |

The default cancellation bodies would all misdescribe a refusal (nobody asked; no supplier desk and no courier recall is involved), which is why the two phases get their own branch. `refunded` is deliberately **not** forked — a completed refund reads the same whoever started it.

**Journey wiring** — `refused_delivery` ("Refused delivery"), `src/data/journeys/refusedDelivery.js`. The happy-path graph plus one extra option at the door: both chain ends (`shipped_out_for_delivery` for AE/ZA, `shipped_simple` for SA/Others) offer `delivered` **or** `delivery_refused`.

```
… → out for delivery / shipped
     ├─ delivered                                  (unchanged happy path)
     └─ delivery_refused          ← cancels the order + stamps the refund
          ├─ refused_cancel_accepted  (SA)  → refused_refunded
          └─ refused_refunded               (AE / ZA / Others)
```

The market fork is per-edge `when: (o) => countryConfig(o).refusalReview` guards mirroring the node's own `apply`, so the dev panel offers exactly one next step. `delivery_refused` still never touches `subStatusId`, so one node serves both country chains. Replay `?journey=refused_delivery&country=SA` for the three-step path, any other country for the two-step. The node fires one event — `shipment.delivery_refused` (unauthored ⇒ silent); a separate "we've cancelled your order" comm would need its own node.

**Journey-mode prerequisite.** `useJourney` now spreads `country` onto the replayed order **before** the first `apply`, so a node's `apply` and its edge `when(order)` guards can read `countryConfig(o)` — without it both fell back to `AE` whatever `?country=` said, and the fork above silently never fired. `App.jsx` re-injects the same value downstream, so nothing else changes (see [journey_backend_spec.md](./journey_backend_spec.md)).

## 3. Refund-hero card (`PastOrderCard` — cancelled branch)

The refund-hero card leads with the **refund** as the visual hero rather than the fulfilment journey. The whole card is wrapped in `OrderClaimLink` (§10 of [claim_tracking.md](./returns/claim_tracking.md)) so a cancelled order, like a claim, gets the paired order half (a neutral `PreCancellationOrderCard`, since the order never arrived) + connector rail; tapping either compact row flips the accordion between the order and the cancellation half. A `w-1` left accent strip carries the phase tone (warn amber for `requested`, brand purple for `refund_pending`, success green for `refunded`). The top eyebrow now reads `#{cancellationRef}` (the cancellation ref — `Cancellation` when absent), since the wrapper's strip already owns the `Order #{id}`; the phase pill sits on its own row below, wrapped in a `StatusExplainer` that shows an inline "ⓘ Learn more" accordion with a plain-language stage definition (copy from `STATUS_EXPLANATIONS` in `lib/statuses.js` under the `cancellation_<cancellationStatusId>` namespace — one entry per phase); then a tinted hero block whose internal eyebrow reads simply `Cancellation` (the ref moved up to the card eyebrow) — parallel to `ClaimCard`'s `Claim · {type}` eyebrow. Below that, a smaller `Refund of` / `Refunded` label sits above the refund amount (`text-[28px]` tabular-nums) and a destination chip — wallet destinations get a brand→accent gradient chip (echoes the `GreetRow` credits pill) that, when an `onOpenWallet` handler is threaded, is a tappable button (chevron, `stopPropagation`) opening the `WalletSheet` ledger (see [wallet.md](./wallet.md)); card destinations get a neutral chip. BNPL destinations (`refund.destination.kind === 'bnpl'`) render the same neutral chip but with just the provider brand as the label (no `•• last4`), and an inline `BnplDisclaimerTooltip` Info-icon (`stopPropagation` because the header is the tap target) opens a popover: "{provider} may charge additional fees on refunded purchases. Check your {provider} account for details." Refunded orders surface a `fundsAvailable` sub-copy line ("Available now in your wallet"); the two earlier phases make no ETA promise. A `requested` cancellation on a stalled **`shipped`** order (§2.6) closes the hero with a dashed-divided warn-tone `Truck` caveat row — *"Subject to the courier stopping your parcel — we'll confirm within 48 hours"* — because on that path the refund is conditional on the recall succeeding; it's gated on `requested && statusId === 'shipped'`, so created/QC cancellations are unchanged. On a **refused** delivery (`deliveryRefused`, §2.7) the same row names what the step is actually waiting on and quotes no window: *"The refused parcel is on its way back to us, and our local team is signing the cancellation off."* — a variant only `refusalReview` markets can reach, since they are the only ones whose refusal path has a `requested` step at all. When the refund carries a late-promise Wallet bonus (`refund.bonus > 0`, see §2.5), a success-tone *"Includes AED 100 Wallet bonus"* line (`Sparkles`) renders under the hero amount on every phase. `RefundDetailsSheet` shows the bonus as a `Subtotal` → `Revibe Wallet bonus +AED 100` (success tone) block parallel to the card-refund fee block, with `Total refund` = `subtotal + bonus`.
...
Expanded reveals a numbered dot stepper titled "Cancellation progress" (mirrors `ClaimCard`'s "Claim progress" header for symmetry). Every **customer-raised** cancellation renders the same three steps (`Requested` / `Pending` / `Refunded`); the difference is timing — on the created-stage path the `requested` step ticks over instantly (no supplier check needed), while on the `quality_check` path it waits on supplier confirmation. A **refused delivery** outside a `refusalReview` market renders **two** (`Pending` / `Refunded`) — the step list comes from `cancellationStepsFor(order)` and the current index from `cancellationProgressIndex(order)`, which searches that list rather than the constant (§2.7). Each reached/current step carries the timestamp it entered that phase underneath its label (sourced from `order.cancellationTimeline[step.id]`); upcoming steps render the label only.
...
Once an order has been cancelled but the refund hasn't been accepted yet (so the card lives in **In progress** as a `PastOrderCard` refund-hero variant — `cancellationStatusId` is `requested`), the expanded view carries a primary brand-purple `I want to keep my order` button stacked **above** the existing `View refund details` button. The button is gated on `requested` **and** `!order.deliveryRefused` (`canKeep`): once the refund is accepted (`refund_pending`) or credited (`refunded`) the affordance disappears, because the cancellation is committed and reversing it is no longer a simple cancel-the-cancellation operation — and on a refused delivery it never appears at all, because the customer never asked for the cancellation and the parcel is already travelling back (§2.7).
...
A **reversed** cancellation (the customer used *Keep my order*) leaves the same kind of trace: `cancellationTimeline.reverted` flips `buildCancellationEvent` to emit a neutral reversal chip, labelled `Cancel reversed` by `HistoryThread`'s `chipLabel` (ref + reason from `order.cancellationReversal`). That label case was missing until the stuck-in-transit review landed, so a kept-and-delivered order used to read `Cancelled` — actively wrong on an order that was never cancelled. Because the kept order resumes to delivery, the `delivered` mode of `getHistoryEvents` now also surfaces this cancellation trace (it previously returned only `Order placed`) — so both reversed and declined cancellations read honestly on the eventual `DeliveredOrderCard`.
...
**Dissuade fires at `created` + `quality_check` + `shipped`.** A `shipped` order only ever reaches the sheet via the stuck-in-transit route (§2.6), which is always `promiseBreached` — so it lands on the §2.5 apology variant rather than the fee-waiver pitch. Everywhere else `shipped` has no `Cancel order` affordance at all, and once the order is `delivered` cancellation isn't a concept — returns take over.
...
| `cancellationStatusId` *(optional)* | enum | One of `requested`, `refund_pending`, `refunded`. Drives the refund-hero card's phase tone, section routing (In progress vs Past orders), the 3-step Cancellation progress stepper, and the *keep my order* window (`requested` only). Combined with `statusId` it also selects the stage-specific explainer copy (`cancellation_{phase}_{statusId}` → falls back to `cancellation_{phase}`) — see §2.6. |
...
| `cancellationTimeline.reverted` *(optional)* | string | Human-readable timestamp at which the customer reversed the cancellation via *Keep my order*. Presence of this key makes `buildCancellationEvent` emit a reversal chip (tone neutral, labelled `Cancel reversed`). Takes precedence over `rejected` / `requested` in the builder. |
...
### 7.4 Late + past-promise / refused-delivery flags

| Field | Type | Notes |
|---|---|---|
| `deliveryRefused` *(optional)* | boolean | Marks a cancellation as **raised by the refusal scan rather than the customer** (§2.7). It is *not* a self-cancel gate — `canCancelShipped` no longer reads it, because the node that sets it also sets `state: 'cancelled'` in the same breath. Its four readers: the two refusal `statusDescription` bodies, the `cancellation_{phase}_refused` explainer keys, `cancellationStepsFor`'s two-step fork (with `countryConfig(order).refusalReview`), and `canKeep` (suppresses *keep my order*). Not interchangeable with `promiseBreached` — no Wallet bonus, no apology screen. In the prototype it's stamped by the `delivery_refused` journey node; **production** reads the courier's refusal scan. |
...
│   ├── HistoryThread.jsx             Drives the Cancel requested / rejected / reversed chips on layered cards
│   ├── StatusExplainer.jsx           ⓘ Learn more accordion beside the phase pill (stage-specific copy, §2.6)
...
└── lib/
    ...
    ├── returns.js                    SHIPPED_CANCEL_WINDOW_DAYS (7) + canCancelShipped(order) — the §2.6 gate only (country-gated promiseBreached)
    ├── statuses.js                   CANCELLATION_STATUSES + cancellationStepsFor/cancellationProgressIndex (the §2.7 two-step fork) + statusDescription/statusExplanation (stage- and refusal-specific copy)
    └── countries.js                  shippedCancellation (false in AE, §2.6) + refusalReview (true in SA only, §2.7) capability flags
```
...
- **Stuck-in-transit cancellation is journey-mode only.** There is no static mock for it — the showcase orders are all `AE` (where the flag is off) and none carry `promiseBreached` on `shipped`, so §2.6 is reachable only via `?journey=shipped_cancellation&country=ZA|SA|Others`. The window itself is faked: production must compute "in transit longer than `SHIPPED_CANCEL_WINDOW_DAYS`" from a real shipped timestamp / the EDD model rather than reading the stamped `promiseBreached` flag, and read the 7-day window from backend config. The parcel recall promised on the Confirm step is copy only — no recall is issued, and the accept/decline verdict is a dev-panel branch rather than an ops decision. The **48 hours** in the sheet, the hero caveat row, and the explainer are hardcoded copy in three places; production should read one SLA value from config.
- **Refused delivery is journey-mode only, and has no reversal.** §2.7 is reachable only via `?journey=refused_delivery` (`&country=SA` for the three-step ops-sign-off variant, anything else for the two-step) — no static mock carries `deliveryRefused`. The auto-cancellation is faked at the node: production has to raise it off the courier's refusal scan, decide who signs it off per market (the `refusalReview` flag is our guess at that being SA-only), and decide whether a refused parcel can be **re-attempted** at all — today refusing is terminal, with no re-delivery path and no way back. It also needs the real policy on who eats the return freight and whether the fee waiver survives a customer-caused return. The reversal is deliberately absent rather than broken now: `canKeep` excludes refusals, so no CTA renders for the missing `cancellation_kept` node.
- **Re-requesting after a decline is unbounded.** A declined cancellation returns the order to `open` with `promiseBreached` intact, so `canCancelShipped` shows `Cancel order` again immediately and the customer can ask as many times as they like. Deliberate for the prototype (a parcel still stuck a week later should be cancellable), but production needs the real policy — a cool-off, an attempt cap, or a courier-side "already refused" flag.
- **`I want to keep my order` is a stub outside journey mode.** In the static showcase confirm just closes the sheet. In **journey mode** `onKeep` → `handleKeepOrder` advances the `cancellation_kept` node (reverts to open, voids the refund, resumes fulfilment, leaves a reversed trace) — defined in both the `cancellation` and `shipped_cancellation` journeys. The CTA is gated on `cancellationStatusId === 'requested'` **and** `!order.deliveryRefused`, so any journey that reaches a *customer-raised* `requested` **must** define a `cancellation_kept` node or the button renders and does nothing; `refused_delivery` reaches `requested` (in SA) without one and is safe because the refusal check suppresses the CTA. The window is `requested`-only — production still needs a reverse-cancellation endpoint and the rule for the wider window (e.g. whether `refund_pending` is ever reversible, irreversible the moment funds are released to the processor).
