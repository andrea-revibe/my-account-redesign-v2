...
The screen layout: the device-prep warn callout, an `iPhone` / `Android` OS-tabs control, then a single prominent `Start the guided reset` launcher card (Sparkles icon, `One step at a time · about 10 min` subtitle), a `Can't unlock or power on the device?` helper line (WifiOff icon) pointing at the guide's remote route, and the confirmation checkbox below. The launcher opens `ResetGuideSheet` — **OS-parametrised** via an `os` prop: one shared shell (the phase machine, full-screen `createPortal` panel, progress bar, back chevron, slide transitions, trouble disclosures, `MockCarousel`, `MiniPhone`/`Row`) with per-platform step tables, copy, and mock screens. The intro route-chooser copy is identical on both (*Yes, I can unlock it* / *No — it's broken or won't turn on*); only the hero differs (iOS PNG vs an asset-free CSS Galaxy slab on Android), both under the same brand-bg glow.
