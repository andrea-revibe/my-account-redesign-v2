...
- **`stepError(state)`** (in `flowReducer.js`) is the single source of truth: it returns the **first** unmet requirement for the current step as a stable key (e.g. `claimType`, `subtype`, `description`, `attachment`, `devicePrepOption`, `resetGuide`, `resetConfirm`, `unlink`, `passcode`, `packing`, `address`, `email`, `phone`, `pickupConfirm`, `refundMethod`), or `null` when the step is complete. The order of the checks **is** the "one at a time" reveal order.
...
Two stacked radio cards. Continue stays clickable; a premature click surfaces the step's first unmet requirement inline (§2.1.1) — `devicePrepOption` (no path picked, message above the cards) → then the path-specific gate below. A `If you leave this flow, you'll need to start over` hint sits below.

- **Option A — `I've factory reset the device`** (recommended pill). Carries an `iPhone` / `Android` OS-tabs control and a required confirmation checkbox. The reset instructions split by OS:
  - **iOS** — a `Start the guided reset` launch card (Sparkles icon, `One step at a time · about 10 min` subtitle) opens `ResetGuideSheet`, now a **full-screen guided walkthrough** (`createPortal` to `document.body`, full-height `max-w-mobile` panel, `animate-slideUp`) — one step per screen, each grounded in a CSS-drawn mock iOS screen. Flow: an **intro** route-chooser (iPhone hero + "About 10 minutes" pill → *Yes, I can unlock it* / *No — it's broken or won't turn on*); per-step screens with a mock illustration, short lead, a brand/warn "why" callout, and a collapsible troubleshooting disclosure that can **escalate to the remote path**; and a **done** screen (success tick + an optional final-touches checklist — SIM, Apple Watch, IMEI photo, order number — persisted in `devicePrep.resetGuideChecks`). A segmented progress bar (`Step X of N`), a back chevron, and directional slide transitions sit in the header/body. Two routes: **device** (3 steps — sign out of iCloud → erase all content → restart and check the Hello screen) and **remote** (2 steps — first `icloud.com/find` → **Remove This Device**, *not* Erase; then an alternative `account.apple.com` route framed as *Or unlink it from your account*). The `account.apple.com` step's mock is a **four-screen swipeable carousel** (`AccountRemoveCarousel` — prev/next arrow buttons overlaid at the phone's edges, clickable dot indicators, a per-screen `N. caption`, and the same directional slide animation): menu → **Devices** → pick this iPhone → scroll to **About** → **Remove from account** (glowing). Account-holder details on the mock are masked. The first remote step's why callout explains Erase keeps Activation Lock; its "What if it's still online?" disclosure covers the offline-vs-online + 30-day behaviour and the sign-out-on-device quirk; the second step's "Which one should I use?" disclosure contrasts the two routes. Once the customer reaches the done screen and taps `Done`, `onDone` sets `devicePrep.resetGuideSeen`. After completion the launch card flips to a success-toned `Guided reset completed` state.
  - **Android** — keeps the inline collapsible chapter-style accordion (`How to factory reset`): a vertical numbered chapter list, each with a title, an optional emphasised lead + body, an optional nested sub-action list (bolded UI labels like **Settings** / **Sign Out**), and an optional success-tinted verification tip. The critical sign-out-of-Google chapter carries the "single most important step" lead to surface the FRP risk before the wipe.
- **Option B — `I can't factory reset it`**. Same OS tabs and the same chapter-style accordion guide (3 chapters per OS — sign in to the find-device site, open it and pick the device, erase + remove from account/sign out of Google), followed by a "removed device from account" confirmation toggle, a 6-digit passcode field, and the encryption-disclosure note. The reducer stores `accountUnlinked` + `passcode`; the toggle gates Submit.

**iOS confirm gate.** The Option A confirm checkbox (*"I confirm this device has been unlinked and factory reset."*) is locked until the customer has been through the guide — opened it **and** tapped `Done` (`devicePrep.resetGuideSeen`). This is one slice of the flow-wide soft validation (§2.1.1): `stepError` returns `resetGuide` while the guide is unseen, so a premature `Continue` dispatches `ATTEMPT` and paints the guide launch card red (`border-danger`, danger icon) with the message *"Run the guide above and tap Done to confirm."* under the checkbox (AlertTriangle + `animate-slideDown`) instead of advancing. Once the guide is completed the red state clears, the checkbox unlocks, and the gate falls to `resetConfirm` (confirm checkbox unticked → checkbox border + `InlineError` *"Confirm you've reset the device before continuing."*). The credentials path gates on `unlink` (toggle) then `passcode` (6 digits) the same way. Android has no guide-seen gate (its accordion is inline) so it lands straight on `resetConfirm`.

The reducer stores `devicePrep: { option, os, resetConfirmed, accountUnlinked, passcode, resetGuideChecks, resetGuideSeen }`. Credentials are never persisted back into the claim object — they're masked to `Credentials provided` on Step 7 review and in the `ClaimDetailsSheet`.
