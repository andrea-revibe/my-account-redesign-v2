...
- **Option A — `I've factory reset the device`** (recommended pill). Carries an `iPhone` / `Android` OS-tabs control and a required confirmation checkbox. Both OS tabs launch the **same** `Start the guided reset` card (Sparkles icon, `One step at a time · about 10 min` subtitle) opening `ResetGuideSheet` — now **OS-parametrised** via an `os` prop: one shared shell (the phase machine, full-screen `createPortal` panel, progress bar, back chevron, slide transitions, trouble disclosures, `MockCarousel`, `MiniPhone`/`Row`) with per-platform step tables, copy, and mock screens. The intro route-chooser copy is identical on both (*Yes, I can unlock it* / *No — it's broken or won't turn on*); only the hero differs (iOS PNG vs an asset-free CSS Galaxy slab on Android), both under the same brand-bg glow.
  ...
  - Once the customer reaches the done screen and taps `Done`, `onDone` sets `devicePrep.resetGuideSeen`; the launch card then flips to a success-toned `Guided reset completed` state.
...
**Confirm gate (both platforms).** The Option A confirm checkbox (*"I confirm this device has been unlinked and factory reset."*) is locked until the customer has been through the guide — opened it **and** tapped `Done` (`devicePrep.resetGuideSeen`). Now that both iOS and Android launch the guided sheet, this gate applies to both (previously iOS-only — Android used to land straight on `resetConfirm` via its inline accordion). It's one slice of the flow-wide soft validation (§2.1.1): `stepError` returns `resetGuide` while the guide is unseen, so a premature `Continue` dispatches `ATTEMPT` and paints the guide launch card red (`border-danger`, danger icon) with the message *"Run the guide above and tap Done to confirm."* under the checkbox (AlertTriangle + `animate-slideDown`) instead of advancing. Once the guide is completed the red state clears, the checkbox unlocks, and the gate falls to `resetConfirm` (confirm checkbox unticked → checkbox border + `InlineError` *"Confirm you've reset the device before continuing."*). The credentials path gates on `unlink` (toggle) then `passcode` (6 digits) the same way.
