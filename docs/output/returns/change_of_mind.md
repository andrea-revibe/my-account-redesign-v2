...
**iOS confirm gate.** The Option A confirm checkbox (*"I confirm this device has been unlinked and factory reset."*) is locked until the customer has been through the guide — opened it **and** tapped `Done` (`devicePrep.resetGuideSeen`). The gate is *soft-validated* like the Step 7 acks: `canAdvance` keeps `Continue` clickable on iOS while the guide is unseen, and `ClaimFlow.handlePrimary` intercepts a premature click — it sets `devicePrepAttempted`, paints the guide launch card red (`border-danger`, danger icon), and pops the danger-toned message *"Run the guide above and tap Done to confirm."* under the checkbox (AlertTriangle + `animate-slideDown`) instead of advancing. Once the guide is completed the red state clears, the checkbox unlocks, and `Continue` falls back to the normal `resetConfirmed` gate. Android has no guide-seen gate (its accordion is inline).
