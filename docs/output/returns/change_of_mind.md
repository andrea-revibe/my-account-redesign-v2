...
- **Option A — `I've factory reset the device`** (recommended pill). Carries an `iPhone` / `Android` OS-tabs control and a required confirmation checkbox. The reset instructions split by OS:
  - **iOS** — a `Open the step-by-step reset guide` launch button (BookOpen icon, `~10 min` subtitle) opens `ResetGuideSheet`, a full-screen sub-sheet (`createPortal` to `document.body`, slide-up, `max-h-[94vh]` scrollable). Inside, a `First — how will you do this?` chooser routes to one of two named routes, each a tickable checklist that persists ticks in `devicePrep.resetGuideChecks`: **`Reset from the device`** (brand-toned, "most people" — back up → sign out of iCloud → erase → confirm the Hello screen) and **`Remove it remotely`** (warn-toned — icloud.com/find + appleid.apple.com when biometrics are broken or the device won't power on). Each route can escalate to the other via inline danger/brand callouts. An optional **`Final touches`** checklist (SIM, Apple Watch, iMessage dereg, IMEI photo, order number) sits at the **end** of each route. A footer `Done` button closes the sheet.
