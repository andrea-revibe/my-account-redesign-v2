...
├── artifact/       ← interactive HTML deliverables paired with their spec (e.g. notifications-flow)
...
| [artifact/notifications-flow.md](./artifact/notifications-flow.md) (+ [.html](./artifact/notifications-flow.html)) | Notification **strategy** over the journeys — which channels (mobile push / email / WhatsApp) fire at each `event`, and why. Channel roles + selection rules + a per-event trigger matrix across all journeys (incl. deliberate silences); paired with an interactive current-vs-proposed HTML artifact. Owns the *policy*; `journey_backend_spec.md` owns the *mechanism* |
...
| Notification channel policy — which channels fire at an `event`, silences, adding/re-classing a trigger | [artifact/notifications-flow.md](./artifact/notifications-flow.md) (+ its `.html`) + `CHANGELOG.md` |
